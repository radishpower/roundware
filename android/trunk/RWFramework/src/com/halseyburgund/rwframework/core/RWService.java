...
public class RWService extends Service implements Observer {
	...
	private void startLocationUpdates() {
		if (configuration != null) {
			// only start receiving updates when really needed
			if (configuration.isGeoListenEnabled() || configuration.isGeoSpeakEnabled()) {
	...
	public void stopService() {
		rwSendLogEvent(R.string.rw_et_stop_session, null, null, true);
		stopQueueTimer();
		releasePlayer();
		stopSelf();
	}
	...
	public void setMockLocation(String latitude, String longitude) {
		releaseMockLocation();
		if (!("N/A".equalsIgnoreCase(latitude)) && (!("N/A".equalsIgnoreCase(longitude)))) {
			try {
...
//	}
	 ...
	 */
	public String rwSubmit(RWList tags, String filename, boolean now) throws Exception {
        ...
        RWAction addAssetAction = mActionFactory.createAddAssetToEnvelopeAction(tags, envelopeId, queueFile.getAbsolutePath());
        ...
        if (envelopeId == -1) {
        	...
        	if (now) {
				...
				String url = configuration.getSharingUrl().replace("[id]", envId);
	...
	public void broadcastSharingMessage(String message, String url, String envelopeId, Double latitude, Double longitude, Double accuracy) {
		if (message != null) {
			...
			intent.putExtra(RW.EXTRA_SERVER_MESSAGE, message + " - Link: " + url);
		...
		if (mPlayer == null) {
			...
			mPlayer.setOnErrorListener(new MediaPlayer.OnErrorListener() {
				...
				public boolean onError(MediaPlayer mp, int what, int extra) {
					Log.i(TAG, "MediaPlayer error event");
					return true;
				}
			});
			...
			mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
				...
				public void onCompletion(MediaPlayer mp) {
					Log.i(TAG, "MediaPlayer completion event");
					mp.stop();
