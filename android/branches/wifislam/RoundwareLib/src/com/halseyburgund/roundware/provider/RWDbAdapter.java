 ...
 */
package com.halseyburgund.roundware.provider;
...
import com.halseyburgund.roundware.server.RWActionData;
...
public class RWDbAdapter {
    ...
    private static final String DATABASE_COUNT =
        "select _id from " + DATABASE_TABLE;
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            RWHtmlLog.w(TAG, "Upgrading database from version " + oldVersion + " to "
                    + newVersion + ", which will destroy all old data", null);
    	...
    	try {
    		cursor = mDb.query(DATABASE_TABLE, new String[] {KEY_ROWID, PARAMS}, null, null, null, null, null);
     		...
     		if ((cursor != null) && (cursor.moveToFirst())) {
    			...
    			RWActionData data = new RWActionData(mContext, props);
    			action = new RWAction((long)id, data);   		
    		}
    	}
    	catch (Exception ex) {
    		// not really an error to report
    		ex.printStackTrace();
    		RWHtmlLog.e(ex.getMessage());
    	}  		
