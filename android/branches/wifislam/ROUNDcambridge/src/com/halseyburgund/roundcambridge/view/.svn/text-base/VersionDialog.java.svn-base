 ...
 */
package com.halseyburgund.roundcambridge.view;
 ...
 * based on the version id (since Google also uses this to track application
 * updates).
 * 
 * @author Rob Knapen
 */
public class VersionDialog {
	...
	public static void show(Context context, String packageName, int dialogResId, int text1ResId, boolean forced) {
		...
		if ((forced) || (!currentVersionName.equals(storedVersionName))) {
			Dialog d = createDialog(context, dialogResId, text1ResId);
			if (d != null) {
				d.show();
	...
	private static Dialog createDialog(Context context, int dialogResId, int text1ResId) {
