...
class KLogger
{
	...
	*/
	...
	/**
	 * FALSE to output logging to a logfile (default) or TRUE to use error_log() 
	 * @var boolean
	 */
	private $_useSyslog = FALSE; 
	
	/**
	 * TRUE to include a stacktrace as part of the log. This is *expensive* and should only
	 * be turned on in a development environment during debugging. 
	 * @var boolean
	 */
	 ...
	 * @return KLogger
	 */
	public static function instance($logDirectory = false, $severity = false, $trace = FALSE)
	{
		if ($severity === false) {
		...
		self::$instances[$logDirectory] = new self($logDirectory, $severity, $trace);
	 ...
	 * @return KLogger
	 */
	public static function syslog($severity = false, $trace = FALSE)
	{
		if ($severity === false) {
			$severity = self::$_defaultSeverity;
		}
	
		if (in_array('syslog', self::$instances)) {
			return self::$instances['syslog'];
		}
	
		self::$instances['syslog'] = new self('syslog', $severity, TRUE, $trace);
	 ...
	 * @return void
	 */
	public function __construct($logDirectory, $severity, $useSyslog = FALSE, $trace = FALSE)
	{
		$this->_trace = $trace; 
		
		if ($useSyslog)
		{
			$this->_useSyslog = TRUE; 
	...
	{
		if ($this->_useSyslog && $this->_severityThreshold != self::OFF)
		{
			error_log(preg_replace('/\s+/', ' ', $line)); 
			return;
		}
	...
	private function _getTimeLine($level)
	{
		...
		// syslog adds its own date so we only need a date if we're not using syslog
		if (! $this->_useSyslog)
		{
			$time = date(self::$_dateFormat);
		}
		
		// add the origin of the log call, including file and line #
		if ($this->_trace)
		{
			// 2 is hard-coded to get the line where the log function was originally called
			$l = debug_backtrace(); 
			$time .= $l[2]['file'] . ':' . $l[2]['line']; 
		}
