...
#					self.add_file()
	...
	def add_file (self):
		logging.debug("Composition: ADD FILE")
		self.current_recording = self.recordingCollection.get_recording()
		logging.debug("Composition: ADD FILE 2")
		if not self.current_recording:
			logging.debug("Composition: ADD FILE 3")
			self.state = STATE_WAITING
			return

		logging.debug("Composition: ADD FILE 4")
		duration = min(
			...
			random.randint(
				...
				max(self.comp_settings.maxduration,
					gst.SECOND)))
		logging.debug("Composition: ADD FILE 5")
