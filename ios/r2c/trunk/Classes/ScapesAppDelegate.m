...
*/
	...
	if ([errorMessage length]) {
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"R:C Message" message:errorMessage delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
		[alert show];
		[alert release];
	}
...
- (void)requestFailed_startsession:(ASIHTTPRequest *)request
{
	if (self.networkTryAgainCount == 0) {   // Since trying again usually solves the problem here we do just that.
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Network Error" message:@"Oops! A network error has occurred. If a network connection can not be established R:C will not be able to function properly." delegate:self cancelButtonTitle:nil otherButtonTitles:@"Try Again", @"Cancel", nil];
        [alert show];
        [alert release];
    }
}
