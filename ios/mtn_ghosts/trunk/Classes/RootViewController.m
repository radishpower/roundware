...
- (void)viewDidLoad {
    ...
    NSURL *url = [NSURL URLWithString:@"http://halseyburgund.com/work/mg/"];
    SHKItem *item = [SHKItem URL:url title:@"Check out Mountain Ghosts for iPhone and Android by Halsey Burgund!"];
...
- (IBAction)listen:(id)sender
{
	if ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] networkAvailable]) {
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No Network" message:@"There is no network connection. Please wait a moment and try again. If this persists you can quit Mountain Ghosts and launch it again." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
		[alert show];
		[alert release];
	}
}
...
{
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"About Mountain Ghosts" message:kABOUT_SCAPES delegate:self cancelButtonTitle:nil otherButtonTitles:@"More Info", @"Done", nil];
	[alert show];
	[alert release];
}
...
{
	if (([alertView title] == @"About Mountain Ghosts") && (buttonIndex == 0)) { // Website button
