	   ...
	   didFailWithError:(NSError *)error
{
	...
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Location Error" message:@"An error occurred trying to obtain your location. This could be due to low GPS signal strength. Mountain Ghosts requires location information to function properly. Please restart Mountain Ghosts and allow location." delegate:self cancelButtonTitle:nil otherButtonTitles:@"Try Again", @"Cancel", nil];
	[alert show];
	[alert release];
...
*/
	...
	if ([errorMessage length]) {
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Mountain Ghosts Message" message:errorMessage delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
		[alert show];
		[alert release];
	}
...
- (void)requestFailed_startsession:(ASIHTTPRequest *)request
{
	if (self.networkTryAgainCount == 0) {   // Since trying again usually solves the problem here we do just that.
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Network Error" message:@"Oops! A network error has occurred. If a network connection can not be established Mountain Ghosts will not be able to function properly." delegate:self cancelButtonTitle:nil otherButtonTitles:@"Try Again", @"Cancel", nil];
        [alert show];
        [alert release];
    }
}
