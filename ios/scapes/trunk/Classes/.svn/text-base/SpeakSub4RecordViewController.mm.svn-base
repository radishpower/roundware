...
- (void)viewDidLoad {
	...
	/*
	 if kAudioFormatMPEG4AAC works on 3G then OK
	 otherwise kAudioFormatAppleLossless 
	 convert to WAV
	 */

	// /* kAudioFormatMPEG4AAC /*very small*/ /*kAudioFormatAppleLossless pretty small*//*kAudioFormatLinearPCM large*/
	// Create a new sound recorder
	NSDictionary *recordSettings =
		[[NSDictionary alloc] initWithObjectsAndKeys:
		 [NSNumber numberWithFloat: 22050.0], AVSampleRateKey,
		 //[NSNumber numberWithInt: kAudioFormatAppleLossless /* kAudioFormatMPEG4AAC /*very small*/ /*kAudioFormatAppleLossless pretty small*//*kAudioFormatLinearPCM large*/], AVFormatIDKey,
		 [NSNumber numberWithInt: [(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] audioFormat]], AVFormatIDKey,
...
- (void)viewWillAppear:(BOOL)animated {
	...
	//self.textLabel.text = [speakQuestions objectForKey:keyStr];
...
{
	int currentTime = ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] maxRecordingTimeSeconds]) - [[self soundRecorder] currentTime] + 1; // + 1 so we can be more accurate in our display
...
- (IBAction)pressRecordButton:(id)sender {
	...
	[self updateCountdownLabel: ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] maxRecordingTimeSeconds])];
	...
	if (recording) {
		...
		([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] rememberRecordedCoordinate]);
		([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] submitEvent:kEVENT_STOP_RECORD]);
...
- (void)record {
	...
	[soundRecorder recordForDuration:(NSTimeInterval)([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] maxRecordingTimeSeconds])];
	...
	([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] submitEvent:kEVENT_START_RECORD]);
}
