...
- (IBAction)listen:(id)sender
{
	if ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] networkAvailable]) {
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No Network" message:@"There is no network connection. Please wait a moment and try again. If this persists you can quit Scapes and launch it again." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
		[alert show];
		[alert release];
	}
}
...
{
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"About Scapes" message:kABOUT_SCAPES delegate:self cancelButtonTitle:nil otherButtonTitles:@"Website", @"Done", nil];
	[alert show];
	[alert release];
}
...
{
	if (([alertView title] == @"About Scapes") && (buttonIndex == 0)) { // Website button
