...
NSString * const AS_AUDIO_QUEUE_CREATION_FAILED_STRING = @"Audio streaming error"; //@"Audio queue creation failed.";
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		UIAlertView *alert =
			[[[UIAlertView alloc]
				initWithTitle:NSLocalizedStringFromTable(@"Oops...network error!" /*@"Audio Error"*/, @"Errors", nil)
			    message:[NSString stringWithFormat:@"%@\n\n%@", NSLocalizedStringFromTable([AudioStreamer stringForErrorCode:self.errorCode], @"Errors", nil), @"Please try pressing the pause button and then the play button on the \"Listen\" screen to restart audio or quit the app and relaunch it. Thanks!"]
				delegate:self
...
- (void)stop
{
	@synchronized(self)
	{
		...
		else if (state != AS_INITIALIZED)
		{
			...
			stopReason = AS_STOPPING_USER_ACTION;
			return; // JMZ - without this, you can end up stuck in the while loop below in a constant sleep on the main thread
		}
	}
