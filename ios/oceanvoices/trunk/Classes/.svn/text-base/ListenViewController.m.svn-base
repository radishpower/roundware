	...
	if (currentSubViewController != nil) {
		...
		switch (which) {
			case kListenSub1View:
				...
				// UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithTitle:@"Video" style:UIBarButtonItemStylePlain target:self action:@selector(pressVideoButton:)];          
			...
			case kListenSub3WhatView:
				...
				UIBarButtonItem *anotherButton3 = [[UIBarButtonItem alloc] initWithTitle:@"Done" style:UIBarButtonItemStyleDone target:self action:@selector(pressDoneButton:)];          
				[self.navigationItem setRightBarButtonItem:anotherButton3 animated:YES];
				[anotherButton3 release];
...
{
	if (self.moviePlayer && ([self.moviePlayer respondsToSelector:@selector(loadState)])) {
...
- (IBAction)pressVideoButton:(id)sender {
    ...
    if (moviePlayer == nil) {
		MPMoviePlayerController *mp = [[MPMoviePlayerController alloc] initWithContentURL:[NSURL URLWithString: kMovieURL]];
	...
	if (moviePlayer) {
        ...
        if ([self.moviePlayer respondsToSelector:@selector(loadState)]) { 
            [currentSubViewController.view addSubview:[self.moviePlayer view]];  
...
- (void)updateUserInterface:(NSNumber*)begin {
	if ([begin boolValue] == YES) {
		if (([(OceanVoicesAppDelegate *)[[UIApplication sharedApplication] delegate] isAudioStreamerPlaying]))
			[(OceanVoicesAppDelegate *)[[UIApplication sharedApplication] delegate] toggleAudioStreamer];
	}
	[self pressPlayButton:nil];
}
...
- (void)updateStream {
	if (self.navigationItem.rightBarButtonItem) {
		...
		([(OceanVoicesAppDelegate *)[[UIApplication sharedApplication] delegate] submitEvent:kEVENT_MODIFY_STREAM]);
	}
}
...
- (IBAction)pressPlayButton:(id)sender {
	BOOL isAudioStreamerPlaying = ([(OceanVoicesAppDelegate *)[[UIApplication sharedApplication] delegate] isAudioStreamerPlaying]);
