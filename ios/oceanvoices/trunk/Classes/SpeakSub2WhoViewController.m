...
@implementation SpeakSub2WhoViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return (section == kDemoSection0 && demographicChoices) ? [demographicChoices count] : 0;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == kDemoSection0) {
		...
		NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
		NSMutableArray *myPrefs = [[prefs objectForKey:kSpeakGenderAgePref] mutableCopy];
		if (myPrefs == nil)
		...
		[prefs setObject:myPrefs forKey:kSpeakGenderAgePref];
		[myPrefs release];
