...
- (void)viewDidLoad {
    ...
    NSURL *url = [NSURL URLWithString:@"http://halseyburgund.com"];
    SHKItem *item = [SHKItem URL:url title:@"Check out Stories from Main Street for iPhone."];
...
- (IBAction)listen:(id)sender
{
	if ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] networkAvailable]) {
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"No Network" message:@"There is no network connection. Please wait a moment and try again. If this persists you can quit Stories from Main Street and launch it again." delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
		[alert show];
		[alert release];
	}
}
...
{
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"About\nStories from Main Street" message:kABOUT_SCAPES delegate:self cancelButtonTitle:nil otherButtonTitles:@"More Info", @"Done", nil];
	[alert show];
	[alert release];
}
...
{
	if (([alertView title] == @"About\nStories from Main Street") && (buttonIndex == 0)) { // Website button
