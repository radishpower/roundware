...
@synthesize submittedyn;

#pragma mark -
#pragma mark Overridden Getters

- (NSString*)submittedyn {
	if (submittedyn == nil || [submittedyn length] == 0) {
		NSError* error;
		NSURL* url = [NSURL URLWithString:kSubmittedYNURL];
		NSString* s = [NSString stringWithContentsOfURL:url encoding:NSASCIIStringEncoding error:&error];
		if ([s length] == 1)
			submittedyn = [[NSString stringWithString:s] retain];
		else submittedyn = [[NSString stringWithString:@"N"] retain];
	}
    //NSLog(@"SUBMITTEDYN = %@", submittedyn);
	return submittedyn;
}
...
- (void)enterRecording {
    ...
    [request setPostValue:kProjectID forKey:@"projectid"];
	...
	[request setPostValue:[self submittedyn] forKey:@"submittedyn"];
...
- (void)requestFinished_enterRecording:(ASIHTTPRequest *)request {
	...
	NSLog(@"recordingID: %@", lastRecordingID);
	/*
...
- (void)requestFinished_uploadFile:(ASIHTTPRequest *)request
{
    ...
    ([(ScapesAppDelegate *)[[UIApplication sharedApplication] delegate] submitEvent:kEVENT_MODIFY_STREAM]); // new line to reset bucket of available recordings so that it is heard immediately in MoMS
...
- (void)dealloc {
    ...
    [submittedyn release]; submittedyn = nil;
    [super dealloc];
}
